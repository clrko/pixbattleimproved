    ...
    if (!result[0]) {
      ...
      return bcrypt.genSalt(saltRounds, (err, salt) => {
        ...
        bcrypt.hash(myPlaintextPassword, salt, (err, hash) => {
          ...
          const insertValues = [req.body.username, email, hash];
            ...
            connection.query(sql, selectValues, (err, result) => {
              ...
              if (req.body.invitationCode) {
                ...
                connection.query(sqlInviteGroup, valuesInviteGroup, (err) => {
                  ...
                  connection.query(sqlInviteBattle, valuesInviteBattle, (err) => {
                    if (err) return next(err);
                  });
                });
              }
              ...
              eventEmitterMail.emit('sendMail', {
                type: 'welcome',
                to: result[0].email,
                subject: `Bienvenue chez PxBattle ${result[0].username}`,
                userName: result[0].username,
              });
