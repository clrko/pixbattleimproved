...
router.post('/battle-creation', checkToken, (req, res, next) => {
  ...
  connection.query(sql, value, (err, battleCreationResult) => {
    ...
    connection.query(sqlBattleRule, [insertBattleRulesValues], (err) => {
      if (err) return next(err);
      const sqlGetGroupUsers =
        'SELECT ug.user_id, u.username, u.email, g.group_name FROM user_group AS ug JOIN user AS u ON ug.user_id = u.user_id JOIN `group` As g ON g.group_id = ? WHERE ug.group_id = ?';
...
router.get('/battle-post/status-user', checkToken, (req, res, next) => {
  const sql = `SELECT *
    ...
    WHERE p.user_id = ?
    AND p.battle_id IN(?)`;
  const values = [req.user.userId, req.query.id];
...
router.get('/battle-post/:groupId/:battleId', checkToken, (req, res, next) => {
  ...
  const valueBattleId = [req.params.battleId, req.user.userId];
  connection.query(sqlHasPosted, valueBattleId, (err, photos) => {
    ...
    const sqlBattleInfos = `SELECT t.theme_name, b.deadline, r.rule_name
      ...
      WHERE b.battle_id = ?`;
    const valueBattleId = [req.params.battleId];
...
router.post('/battle-post/addpicture', checkToken, upload.single('file'), (req, res, next) => {
  const sqlExistingPhoto = 'SELECT * FROM photo WHERE user_id = ? AND battle_id = ?';
  const valuesExistingPhoto = [req.user.userId, req.body.battleId];
  connection.query(sqlExistingPhoto, valuesExistingPhoto, (err, photos) => {
    ...
    const valuesInsertPhoto = [req.file.filename, req.user.userId, req.body.battleId, req.body.groupId];
...
router.put('/battle-post/:photoId', checkToken, (req, res, next) => {
  ...
  const valuesUpdatePhoto = [req.body.photoUrl, req.params.photoId];
...
router.delete('/battle-post', checkToken, (req, res, next) => {
  ...
  const valueDeletePhoto = [req.body.photoId];
...
router.get('/battle-vote/:battleId', checkToken, (req, res, next) => {
  const sqlPhotosBattle = `SELECT * FROM photo AS p
    ...
    WHERE b.battle_id = ?
      AND NOT p.user_id = ?`;
  const battleId = [req.params.battleId, req.user.userId];
...
router.get('/battle-vote/:battleId/status-user', checkToken, (req, res, next) => {
  const sql = `SELECT p.photo_id, p.photo_url, up.vote
    ...
    WHERE up.user_id = ?
    AND p.battle_id = ?`;
  const values = [req.user.userId, req.params.battleId];
  connection.query(sql, values, (err, result) => {
    if (err) return next(err);
    res.status(200).send(result);
  });
...
router.get('/battle-results/:battleId/photos', checkToken, (req, res, next) => {
  const battleId = [req.params.battleId];
  const sqlGetPhotos = `SELECT u.username, a.avatar_url, p.*
    FROM avatar AS a
    JOIN user AS u
      ON u.avatar_id = a.avatar_id
    JOIN photo AS p
  ...
  connection.query(sqlGetPhotos, battleId, (err, photos) => {
    if (err) return next(err);
    const sqlGetUsers = `SELECT u.username, a.avatar_url, up.vote, up.photo_id
      FROM avatar AS a
      JOIN user AS u
        ON u.avatar_id = a.avatar_id
...
router.get('/my-battles', checkToken, (req, res, next) => {
  ...
  WHERE ub.user_id = ?`;
  const sqlGetBattleInformationValues = [req.user.userId];
...
router.get('/my-battles/:groupId', checkToken, (req, res, next) => {
  ...
  const sqlGetBattleInformationValues = [req.user.userId, req.params.groupId];
