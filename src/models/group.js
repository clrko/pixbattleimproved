...
exports.getUserGroups = async (userId) => {
  ...
  const stats = connection.query(sqlGetGroupInformation, userId);
  return stats;
};

exports.getUserCountOfGroups = async (userId) => {
  const sqlNumberGroupsUser = 'SELECT count(*) AS nb_groups FROM user_group WHERE user_id = ?';
  const userCountOfGroups = connection.query(sqlNumberGroupsUser, userId);
...
exports.removeGroupMember = async (groupId, userId) => {
  ...
  connection.query(sqlDeleteMember, deleteValues);
};
...
exports.addUserToGroup = async (userId, groupId) => {
  ...
  connection.query(sqlGroupUser, insertValues);
};
