...
const { getGroupMembers, addUserToBattle, getBattleParticipantListWithScores, getBattleParticipantVictories } = require('../models/member');
const { updatePhoto, uploadPhoto, deletePhoto, getBattlePhotosForVote, insertUserVotes, getBattlePhotos } = require('../models/photo')
...
module.exports = {
    ...
    async getAndSendRuleList(req, res, next) {
        try {
    ...
    async createNewBattleAndNotifyUsers(req, res, next) {
        try {
    ...
    async getUserPostedPhoto(req, res, next) {
        try {
    ...
    async getAndSendUserPostedPhoto(req, res, next) {
        try {
    ...
    async getAndSendUserBattlePostInformation(req, res, next) {
        try {
            const battleId = req.params.battleId
    ...
    async uploadPhotoAndSendId(req, res, next) {
        try {
    ...
    async getAndSendPhotosToVote(req, res, next) {
        try {
    ...
    async getAndSendBattleVoteStatus(req, res, next) {
        try {
    ...
    async getAndSendUserVoteStatus(req, res, next) {
        try {
    ...
    async insertUserVotes(req, res, next) {
        try {
            const votes = [
                req.user.userId,
                req.body.photoId1,
    ...
    async getAndSendParticipantBattleResults(req, res, next) {
        try {
            const battleId = [req.params.battleId];
    ...
    async getAndSendPhotoBattleResults(req, res, next) {
        try {
            const battleId = [req.params.battleId];
            const resultsPhotos = await getBattlePhotos(battleId)
    ...
    async getAndSendUserAllBattles(req, res, next) {
        try {
            const userId = req.user.userId;
    ...
    async getAndSendUserGroupBattles(req, res, next) {
        try {
            const userId = req.user.userId;
    ...
    async getAndSendPendingGroupBattle(req, res, next) {
        try {
            const groupId = req.params.groupId
